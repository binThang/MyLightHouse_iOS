...
extension FindPasswordWithPhoneNumberViewController {
    ...
    @objc func handleVerification() {
        ...
        guard let username = usernameTextfield.text, let phoneNumber = phoneNumberTextfield.text, let dateOfBirth = dateOfBirthTextfield.text else {
            return
        }
        ...
        APIService.instance.findPasswordWithPhoneNumber(withUsername: username, withPhoneNumber: phoneNumber, withDOB: dateOfBirth) { (result) in
            if result == "true" {
